...
소듐은 일반적인 암호화 라이브러리와 그렇게 크게 거리감을 두고 얘기할 부분은 아닌 것같습니다. 다만, 확실히 대비되도록 어떤 장점이 있냐고 하면 저는 `soduim_malloc`을 둘 수 있을 것같습니다. 

`sodium_malloc`은 다음과 같은 과정으로 생성됩니다:
1. 페이지 단위로 3개의 연속된 페이지를 할당받습니다. 총 메모리 크기는 제가 아는 선에서 12KB에서 48KB까지 다양할 것입니다.
...
저희가 주로 사용하게 될 함수는 `secret.Do`입니다. 이 함수는 인자로 받은 익명 함수를 실행할 때 할당된 레지스터와 스택이 스코프가 종료될 때 확실하게 `0`으로 밀고 해제됩니다. 또한 힙에 할당된 메모리는 GC가 가져갈 때 확정적으로 `0`으로 밀립니다. 이 동작은 패닉이 발생해도 무시하고 실행합니다.
...
이 글과 연관된 벤치마크 자료도 있으나 자료 자체를 보여드릴 순 없고, 일반적인 사용 상황에서 `sodium-native`가 수동으로 `mprotect`를 사용하는 식으로 메모리를 관리해도 기반이 C++이기에 생각보다 매우 좋은 성능을 보여준다는 것과 생각보다 `memguard.Enclave`가 많이 느리다는 것만 작성할 수 있을 것같습니다. 당연하지만 `memguard.LockedBuffer`는 `sodium-native`의 그것과 비교해서 상당히 빠릅니다.
