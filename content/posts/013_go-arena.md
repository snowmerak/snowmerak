...
어디까지나 아레나의 활용도는 일련의 기능을 실행함에 있어 힙에 할당되는 메모리를 하나의 연속된 공간에 기록하고, 한번에 할당 해제하는 것으로 GC 부담을 줄이는 것입니다. 의의를 GC가 동작하지 않아, 로직이 실행되는 동안 프로그램이 멈추는 일이 없다는 것과 하나의 아레나가 해제되는 동안 다른 로직에 영향을 끼치지 않는다는 것으로, 다소의 메모리 파편화나 할당 해제의 코스트는 비교적 후순위로 평가됩니다. 하지만 아래 벤치마크는 편의성을 위해 장점 외의 부분도 포함하여 작성되었습니다. 
