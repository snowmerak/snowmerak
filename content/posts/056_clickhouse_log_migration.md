...
```
일일 로그 100GB → 최소 4-8노드 클러스터 필요
일일 로그 1TB → 16-32노드 클러스터 + 512GB+ RAM
```

대규모 샤드를 가진 클러스터에서는 GC(Garbage Collection) 압력까지 더해져 moderate한 쿼리 부하에서도 성능 저하가 발생합니다. [^5]
...
| 저장소 | 일일 100GB 기준 월간 비용 (AWS) |
|--------|-------------------------------|
| ES (m5.xlarge × 3노드 + EBS) | 약 $2,500/월 |
| ClickHouse (c5.2xlarge × 2노드 + S3) | 약 $600/월 |
...
```
ES (compressed): 100GB → 실제 저장 80GB
ClickHouse:      100GB → 실제 저장 20-30GB
```
...
**Hot Query (캐시 warmed 상태):** 1.7x~2.6x 더 빠릅니다. [^3]
...
| RAM 총 필요량 (동일 처리량) | 256~512GB | 64~128GB |

ES는 전역 인덱싱을 위해 많은 양의 힙 메모리를 사용합니다. JVM 힙 크기는 32GB를 넘지 않는 것이 권장되며, 이는 ES가 인덱스를 메모리에 최대한 올려놓으려는 전략 때문입니다. [^5]
...
동일한 처리량(일일 1TB 로그, 30일 보관) 기준 월간 비용 비교(AWS us-east-1 기준):

#### ElasticSearch 구성
```
m5.4xlarge × 3노드 (ES 클러스터)    : $864/월
EBS gp3 10TB × 3볼륨               : $1,200/월
데이터 전송 (인트라클러스터)         : ~$100/월
------------------------------------------
총합                              : ~$2,164/월
```
...
#### ClickHouse 구성
```
c5.2xlarge × 2노드 (CH 클러스터)   : $350/월
S3 스토리지 3TB (원시 데이터)       : $72/월
EBS gp3 1TB (캐시용)               : $50/월
------------------------------------------
총합                              : ~$472/월
```

**월간 약 $1,692 절약 = 연간 약 $20,300**
...
✅ **경제성**: 스토리지 비용 10~20x 절감 [^4], 하드웨어 비용 70-80% 절감  
